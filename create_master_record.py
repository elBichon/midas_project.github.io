...
if __name__ == "__main__":
	...
	sql = "SELECT name, MIN(fft_100_close) FROM unhourled_record GROUP BY name"
	table_rows = utils.extract_data(sql,mydb)
	min_index_list = []
	i = 0
	while i < len(table_rows):
		sql = "SELECT id FROM unhourled_record WHERE name = '"+str(table_rows[i][0])+"' AND fft_100_close LIKE "+str(table_rows[i][1]).replace('.0','')
		utils.get_index(sql,min_index_list,mydb)
		i += 1

	sql = "SELECT name, MAX(fft_100_close) FROM unhourled_record GROUP BY name"
	table_rows = utils.extract_data(sql,mydb)
	max_index_list = []
	i = 0
	while i < len(table_rows):
	...
	sql = "SELECT * FROM unhourled_record"
	table_rows = utils.extract_data(sql,mydb)
